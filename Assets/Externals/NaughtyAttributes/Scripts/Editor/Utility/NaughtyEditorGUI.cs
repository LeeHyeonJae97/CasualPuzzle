...
namespace NaughtyAttributes.Editor
{
	public static class NaughtyEditorGUI
	{
		...
		public const float HorizontalSpacing = 5.0f;

		private static GUIStyle _buttonStyle = new GUIStyle(GUI.skin.button) { richText = true };

		private delegate void PropertyFieldFunction(Rect rect, SerializedProperty property, GUIContent label, bool includeChildren, bool valid = true);
		...
		private static void DrawPropertyField(Rect rect, SerializedProperty property, GUIContent label, bool includeChildren, bool valid = true)
		{
			if (valid)
			{
		...
		private static void DrawPropertyField_Layout(Rect rect, SerializedProperty property, GUIContent label, bool includeChildren, bool valid = true)
		{
			if (valid)
			{
		...
		private static void PropertyField_Implementation(Rect rect, SerializedProperty property, bool includeChildren, PropertyFieldFunction propertyFieldFunction)
		{
			...
			else
			{
				...
				string[] errorMsgs = new string[validatorAttributes.Length];
				bool valid = true;

				for (int i = 0; i < validatorAttributes.Length; i++)
				{
					string msg = validatorAttributes[i].GetValidator().ValidateProperty(property);
				...
				{
					propertyFieldFunction.Invoke(rect, property, PropertyUtility.GetLabel(property), includeChildren, valid);
				}

				// if not valid, draw warning helpbox
				if (!valid)
				{
					for (int i = 0; i < errorMsgs.Length; i++)
					{
						if (errorMsgs[i] != null)
							HelpBox_Layout(errorMsgs[i], MessageType.Error, context: property.serializedObject.targetObject);
					}
				}
		...
		public static void Button(UnityEngine.Object target, MethodInfo methodInfo)
		{
			...
			if (methodInfo.GetParameters().All(p => p.IsOptional))
			{
				...
				GUILayout.Space(buttonAttribute.UpperSpace);
				...
				GUILayout.Space(buttonAttribute.UnderSpace);
