	...
	public class NaughtyInspector : UnityEditor.Editor
	{
		...
		protected void DrawSerializedProperties()
		{
			...
			foreach (var group in GetFoldoutProperties(_serializedProperties))
			{
				...
				{
					_foldouts[group.Key] = new SavedBool($"{target.GetInstanceID()}.{group.Key}", true);
				}
		...
		private static IEnumerable<IGrouping<string, SerializedProperty>> GetFoldoutProperties(IEnumerable<SerializedProperty> properties)
		{
			var groups = properties
				...
				.OrderBy(p =>
				{
					FoldoutAttribute attribute = PropertyUtility.GetAttribute<FoldoutAttribute>(p);
					return attribute.InnerPriority;
				})
				.GroupBy(p =>
				{
					FoldoutAttribute attribute = PropertyUtility.GetAttribute<FoldoutAttribute>(p);
					...
					{
						SerializedProperty prop = p.serializedObject.FindProperty(attribute.Name);
						...
						return $"[{p.serializedObject.FindProperty(attribute.Name).stringValue}]";
					}
					else
					{
						return attribute.Name;
